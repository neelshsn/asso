import { getTranslations } from "next-intl/server";
import { Button } from "@/components/ui/button";
import { Link } from "@/lib/navigation";

export default async function AssociationDashboardPage() {
  const t = await getTranslations("dashboards");

  return (
    <section className="bg-gradient-to-b from-cream to-white py-16">
      <div className="mx-auto flex max-w-3xl flex-col gap-6 rounded-[32px] bg-white/90 p-10 text-center shadow-[0_30px_120px_rgba(0,0,0,0.08)]">
        <div className="space-y-3">
          <p className="text-xs uppercase tracking-[0.35em] text-turquoise">
            {t("common.eyebrow")}
          </p>
          <h1 className="font-display text-4xl text-ink">
            {t("association.title")}
          </h1>
          <p className="text-ink/70 text-lg">{t("association.description")}</p>
        </div>
        <p className="text-ink/60 text-sm">{t("common.subtitle")}</p>
        <div>
          <Button
            asChild
            className="rounded-full bg-orange px-8 text-white hover:bg-salmon"
          >
            <Link href="/">{t("common.cta")}</Link>
          </Button>
        </div>
      </div>
    </section>
  );
}
